...
for (let i = 0; i < phrases.length; i++) 
{
    ...
    document.write('<p>cette phrase de longeur '+phrases[i].length+'</p>');
    document.write('<br>');
    if (phrases[max].length < phrases[i].length)
    {
        max = i;
    }
}

document.write('La phrase la plus longue est "'+phrases[max]+ '" de longueur '+ phrases[max].length+' caractères');